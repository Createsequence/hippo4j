...
public interface GlobalThreadPoolPluginManager extends ThreadPoolPluginRegistrar {
    ...
    /**
     * Use plugin for all {@link ThreadPoolPluginSupport}.
     *
     * @param pluginId plugin id
     * @return true if the plugin has been registered, false otherwise
     */
    default boolean enableThreadPoolPluginForAll(String pluginId) {
     ...
     * @return true if the plugin has been registered, false otherwise
     */
    default boolean enableThreadPoolPlugin(String pluginId, String... threadPoolIds) {
        ThreadPoolPlugin plugin = getThreadPoolPlugin(pluginId);
        if (Objects.isNull(plugin)) {
            return false;
        }
        if (ArrayUtil.isNotEmpty(threadPoolIds)) {
            Arrays.stream(threadPoolIds)
                    ...
                    .forEach(support -> support.tryRegister(plugin));
        }
        return true;
    }
     ...
     * @return true if the plugin has been registered, false otherwise
     */
    default boolean disableThreadPoolPluginForAll(String pluginId) {
        ThreadPoolPlugin plugin = getThreadPoolPlugin(pluginId);
        if (Objects.isNull(plugin)) {
            return false;
        }
        getAllThreadPoolPluginSupports().forEach(support -> support.unregister(pluginId));
        return true;
    }
     ...
     * @return true if the plugin has been registered, false otherwise
     */
    default boolean disableThreadPoolPlugin(String pluginId, String... threadPoolIds) {
        ThreadPoolPlugin plugin = getThreadPoolPlugin(pluginId);
        if (Objects.isNull(plugin)) {
            return false;
        }
    ...
    /**
     * Enable registrar for all {@link ThreadPoolPluginSupport}.
     *
     * @param registrarId registrar id
     * @return true if the registrar has been registered, false otherwise
     */
    default boolean enableThreadPoolPluginRegistrarForAll(String registrarId) {
    ...
    /**
     * Enable registrar for specific {@link ThreadPoolPluginSupport}.
     *
     * @param registrarId registrar id
     * @param threadPoolIds thread pool ids
     * @return true if the registrar has been registered, false otherwise
     */
    default boolean enableThreadPoolPluginRegistrar(String registrarId, String... threadPoolIds) {
        ThreadPoolPluginRegistrar registrar = getThreadPoolPluginRegistrar(registrarId);
        if (Objects.isNull(registrar)) {
            return false;
        }
